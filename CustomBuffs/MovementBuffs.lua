...
---@class MovementBuffs: AceModule, AceEvent-3.0
local MB = KitnEssentials:NewModule("MovementBuffs", "AceEvent-3.0")
...
-- Module state
MB.trackerFrames = {}
MB.containerFrame = nil
MB.activeTimers = {}

function MB:UpdateDB()
    self.db = KE.db.profile.MovementBuffs
end
...
function MB:GetDefaults()
    return self.db and self.db.Defaults or {}
end

function MB:GetTrackerConfig(tracker)
...
function MB:CreateTrackerFrame(trackerIndex, config)
    local frame = CreateFrame("Frame", "KE_MovementBuff_" .. trackerIndex, self.containerFrame or UIParent)
...
function MB:ShowTracker(trackerIndex, isPreview)
...
function MB:OnSpellCast(event, unit, _, spellID)
...
function MB:ApplySettings()
    self.db = KE.db.profile.MovementBuffs
    if self.db and self.db.Enabled then
        if not self:IsEnabled() then
            KitnEssentials:EnableModule("MovementBuffs")
        else
            self:CreateAllTrackers()
        end
    else
        if self:IsEnabled() then
            KitnEssentials:DisableModule("MovementBuffs")
        end
    end
end

function MB:RegWithEditMode()
    if KE.EditMode and not self.editModeRegistered then
        KE.EditMode:RegisterElement({
            key = "MovementBuffs", displayName = "Personal Movement Buffs", frame = self.containerFrame,
            ...
            guiPath = "MovementBuffs",
        })
        self.editModeRegistered = true
    end
end

function MB:PreviewAll()
