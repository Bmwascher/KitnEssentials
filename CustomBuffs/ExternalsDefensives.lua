...
---@class ExternalsDefensives: AceModule, AceEvent-3.0
local ED = KitnEssentials:NewModule("ExternalsDefensives", "AceEvent-3.0")
...
-- Module state
ED.trackerFrames = {}
ED.containerFrame = nil
ED.activeTimers = {}

function ED:UpdateDB()
    self.db = KE.db.profile.ExternalsDefensives
end

function ED:OnInitialize()
    self:UpdateDB()
    self:SetEnabledState(false)
end

function ED:GetDefaults()
    return self.db and self.db.Defaults or {}
end

function ED:GetTrackerConfig(tracker)
...
function ED:CreateTrackerFrame(trackerIndex, config)
    local frame = CreateFrame("Frame", "KE_ExternalDef_" .. trackerIndex, self.containerFrame or UIParent)
...
function ED:ShowTracker(trackerIndex, isPreview)
...
function ED:OnSpellCast(event, unit, _, spellID)
...
function ED:ApplySettings()
    self.db = KE.db.profile.ExternalsDefensives
    if self.db and self.db.Enabled then
        if not self:IsEnabled() then
            KitnEssentials:EnableModule("ExternalsDefensives")
        else
            self:CreateAllTrackers()
        end
    else
        if self:IsEnabled() then
            KitnEssentials:DisableModule("ExternalsDefensives")
        end
    end
end

function ED:RegWithEditMode()
    if KE.EditMode and not self.editModeRegistered then
        KE.EditMode:RegisterElement({
            key = "ExternalsDefensives", displayName = "Personal Defensives", frame = self.containerFrame,
            ...
            guiPath = "ExternalsDefensives",
        })
        self.editModeRegistered = true
    end
end

function ED:PreviewAll()
