...
describe("MPT.BuildTimerTemplate", function()
    it("covers every TimerFormat with its widest display string", function()
        assert.are.equal("88:88.888 / 88:88", MPT.BuildTimerTemplate("ELAPSED_TOTAL"))
        assert.are.equal("88:88.888",         MPT.BuildTimerTemplate("ELAPSED"))
        assert.are.equal("88:88.888 (88:88 / 88:88)", MPT.BuildTimerTemplate("ELAPSED_DETAIL"))
        assert.are.equal("88:88",             MPT.BuildTimerTemplate("REMAINING"))
        assert.are.equal("88:88 / 88:88",     MPT.BuildTimerTemplate("REMAINING_TOTAL"))
    end)

    it("falls back to the default format for nil/unknown modes", function()
        assert.are.equal("88:88.888 / 88:88", MPT.BuildTimerTemplate(nil))
        assert.are.equal("88:88.888 / 88:88", MPT.BuildTimerTemplate("BOGUS"))
    end)
end)
