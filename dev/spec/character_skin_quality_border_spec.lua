...
describe("Character skin: the rarity border refusal", function()
    it("paints when the key is absent from an existing section", function()
        assert.is_true(loadSkin()._QualityBordersOn({}))
    end)
    ...
    it("paints when the key is explicitly true", function()
        assert.is_true(loadSkin()._QualityBordersOn({ SlotQualityBorders = true }))
    end)
