...
describe("KE:GetTextOverlayInset", function()
    ...
    it("overhangs right or left only, pushed from a right or left anchor", function()
        local rl, rr, rt, rb = inset("RIGHT", "RIGHT", 12, 0)
        assert.equals(0, rl); assert.equals(12, rr)
        assert.equals(0, rt); assert.equals(0, rb)

        local ll, lr, lt, lb = inset("LEFT", "LEFT", -12, 0)
        assert.equals(12, ll); assert.equals(0, lr)
        assert.equals(0, lt); assert.equals(0, lb)
    end)

    it("overhangs top or bottom only, pushed from a top or bottom anchor", function()
        local tl, tr, tt, tb = inset("TOP", "TOP", 0, 12)
        assert.equals(0, tl); assert.equals(0, tr)
        assert.equals(12, tt); assert.equals(0, tb)

        local bl, br, bt, bb = inset("BOTTOM", "BOTTOM", 0, -12)
        assert.equals(0, bl); assert.equals(0, br)
        assert.equals(0, bt); assert.equals(12, bb)
    end)
