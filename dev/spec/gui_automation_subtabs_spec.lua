...
describe("GUI-Automation: subtab id coverage", function()
    ...
    it("registers a builder for every declared id", function()
        assert.is_function(GUIFrame.registeredContent["AutomationGeneral"])
        assert.is_function(GUIFrame.registeredContent["AutomationInterface"])
        assert.is_function(GUIFrame.registeredContent["AutomationQuests"])
        assert.is_function(GUIFrame.registeredContent["AutomationVendors"])
    end)

    it("negative control: an invented id has no registered builder", function()
        assert.is_nil(GUIFrame.registeredContent["AutomationBogus"])
    end)
end)
