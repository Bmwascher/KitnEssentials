...
local function serializeTrashData()
    ...
    w("-- ║  from the upstream reference data tables.                ║")
...
local function serializeTrashTraits()
    ...
    w("-- ║  DO NOT EDIT BY HAND — re-run the extractor to refresh.  ║")
    w("-- ║                                                          ║")
    w("-- ║  Per-mob identity fingerprints, ported verbatim from the ║")
    w("-- ║  upstream reference's trait rows; the packed hex         ║")
    w("-- ║  identity is decoded by the ported bitfield decoder.     ║")
