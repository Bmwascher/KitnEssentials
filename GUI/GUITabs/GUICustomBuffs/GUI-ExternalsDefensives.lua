...
local function GetModule()
    if KitnEssentials then
        return KitnEssentials:GetModule("ExternalsDefensives", true)
    end
    return nil
end
...
GUIFrame:RegisterContent("ExternalsDefensives", function(scrollChild, yOffset)
    local db = KE.db and KE.db.profile.ExternalsDefensives
    ...
    GUIFrame.contentCleanupCallbacks["ExternalsDefensives"] = function()
    ...
    -- Card 1: Enable
    ----------------------------------------------------------------
    local card1 = GUIFrame:CreateCard(scrollChild, "Personal Defensives", yOffset)

    local row1 = GUIFrame:CreateRow(card1.content, 36)
    local enableCheck = GUIFrame:CreateCheckbox(row1, "Enable Personal Defensives", db.Enabled ~= false,
        function(checked)
            db.Enabled = checked
            ApplyModuleState(checked)
            UpdateAllWidgetStates()
        end,
        true, "Personal Defensives", "On", "Off")
