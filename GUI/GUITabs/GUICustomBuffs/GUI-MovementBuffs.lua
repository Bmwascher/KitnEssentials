...
local function GetModule()
    if KitnEssentials then
        return KitnEssentials:GetModule("MovementBuffs", true)
    end
    return nil
end
...
GUIFrame:RegisterContent("MovementBuffs", function(scrollChild, yOffset)
    local db = KE.db and KE.db.profile.MovementBuffs
    ...
    GUIFrame.contentCleanupCallbacks["MovementBuffs"] = function()
    ...
    -- Card 1: Enable
    ----------------------------------------------------------------
    local card1 = GUIFrame:CreateCard(scrollChild, "Personal Movement Buffs", yOffset)

    local row1 = GUIFrame:CreateRow(card1.content, 36)
    local enableCheck = GUIFrame:CreateCheckbox(row1, "Enable Personal Movement Buffs", db.Enabled ~= false,
        function(checked)
            db.Enabled = checked
            ApplyModuleState(checked)
            UpdateAllWidgetStates()
        end,
        true, "Personal Movement Buffs", "On", "Off")
